...
public class DeoptStorm {
    ...
    public static void main(String[] args) throws Exception {
        ...
        long deoptCount = lines.stream()
            .filter(l -> l.contains("Uncommon trap") && l.contains("deoptStorm") && l.contains("unstable_if"))
            .count();
        if (deoptCount > MAX_DEOPT_LINES) {
            System.out.println(out.getStdout());
            throw new RuntimeException("Failed: too many unstable_if deoptimizations in deoptStorm: "
                + deoptCount + " > " + MAX_DEOPT_LINES);
        }
    ...
    public void run() {
        ...
        data[max_index] = 'a';
        iteration = 0;
        // 3_100_000 iterations: ~3M to trigger 3 compile cycles, then ~100K storm deopts.
        // Keeps output manageable (vs 100M which would overflow the capture buffer).
